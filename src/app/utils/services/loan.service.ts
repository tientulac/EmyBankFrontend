...
export class LoanService {
    ...
    delete(id, token): Observable<any> {
      return this.http
        .delete<any>(this.appConfig.QA_API + 'api/v1/loan/'+id, {
          headers: new HttpHeaders().set('Authorization', `Bearer ${token}`),
        })
        .pipe(
          map((z) => {
    ...
    update(req, token): Observable<any> {
      return this.http
        .put<any>(this.appConfig.QA_API + 'api/v1/loan',req, {
          headers: new HttpHeaders().set('Authorization', `Bearer ${token}`),
        })
        .pipe(
          map((z) => {
    ...
    getListByAccountId(id: number, token): Observable<any> {
      return this.http
        .get<any>(this.appConfig.QA_API + 'api/v1/loan/account/?account_id='+ id, {
          headers: new HttpHeaders().set('Authorization', `Bearer ${token}`),
        })
        .pipe(
          map((z) => {
